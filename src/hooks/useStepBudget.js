...
export function useStepBudget(profile) {
  const automatic = Boolean(profile && isPersonalBudget(auth.currentUser) && profile.personalActivityMode !== 'manual' && profile.personalAccrualEnabled !== false);
  ...
  useEffect(() => {
    ...
    const refresh = async (method = 'getToday') => {
      ...
      const includeActiveEnergy = automaticRef.current;
      setReading(old => old?.date === localDay() ? old : null);
      try {
        const next = await readSteps(uid, method, {includeActiveEnergy});
        if (active && auth.currentUser?.uid === uid) setReading(next);
      } catch {
        if (active) setReading(old => ({ ...old, status: 'error' }));
      } finally { pending = false; if (active) { setBusy(false); if (includeActiveEnergy !== automaticRef.current) void refresh(); } }
    };
  ...
  useEffect(() => { automaticRef.current = automatic; void refreshRef.current(); }, [automatic]);
  useEffect(() => {
    const uid = auth.currentUser?.uid;
    if (!stepsAvailable || !uid || !profile?.dailyNorm?.calories || reading?.status !== 'ready' || reading.date !== localDay() || !Number.isSafeInteger(reading.steps)) return;
    let active = true;
  ...
  return { available: stepsAvailable, automatic, reading, busy, budget, history: currentHistory, historyError,
